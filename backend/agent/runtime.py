...
def _system_prompt() -> str:
    return (
        ...
        "as needed.\n"
        "  3. `add_transaction_types` — SKIP for types already listed in the snapshot.\n"
        "Only register transaction types that are new to this model. Register both\n"
        "debit-side and credit-side type names.\n"
...
"REAL-TIME UI: every successful tool call refreshes the Templates, Rules, "
        ...
        "     *** instrumentid / subinstrumentid STEP RULES (MANDATORY) ***\n"
        "       • NEVER create a calc step named 'instrumentid'. It is always\n"
        "         an implicit global on every row — a step is redundant and\n"
        "         is hard-blocked by the validator. Simply use 'instrumentid'\n"
        "         by name wherever you need it.\n"
        "       • ALWAYS create a calc step named 'subinstrumentid'. Choose\n"
        "         source based on the event's data shape:\n"
        "           SCALAR (one subinstrumentid per instrument per date):\n"
        "             {name:'subinstrumentid', stepType:'calc',\n"
        "              source:'event_field', eventField:'EVENTNAME.subinstrumentid'}\n"
        "           NON-SCALAR (multiple subinstrumentids under one instrumentid):\n"
        "             {name:'subinstrumentid', stepType:'calc', source:'collect',\n"
        "              collectType:'collect_by_instrument',\n"
        "              eventField:'EVENTNAME.subinstrumentid'}\n"
        "         If unsure: check loaded event data. Any instrument with >1\n"
        "         distinct subinstrumentid → non-scalar. Otherwise → scalar.\n"
        ...
        "         NOTE: do NOT create a step for 'instrumentid' (hard-blocked).\n"
        "         DO always create a 'subinstrumentid' step:\n"
        "           source:'event_field'  → scalar (1 sub-id per instrument per date)\n"
        "           source:'collect', collectType:'collect_by_instrument' → non-scalar\n"
        "         (postingdate/effectivedate steps may use source:'value', value:'postingdate'\n"
        "          to alias the global.)\n"
