...
// Rail and panel geometry, shared with the Dashboard so both side docks
// move on the same clock.
export const RAIL_WIDTH = 44;
export const PANEL_WIDTH = 280;
export const PANEL_MS = 260;
export const PANEL_EASE = 'cubic-bezier(0.22, 1, 0.36, 1)';
...
const LeftSidebar = ({ events, selectedEvent, onEventSelect, onDownloadEvents, onImportSuccess, onViewEventData, collapsed = false, onToggleCollapsed }) => {
  ...
  // One shell for both states, so `width` is a transition the browser can
  // actually run: React keeps this element across the toggle. The content
  // inside snaps to its own width immediately and the shell's clip reveals
  // or hides it, which keeps the main column's slide smooth.
  const width = collapsed ? RAIL_WIDTH : PANEL_WIDTH;

  const toggleTab = onToggleCollapsed ? (
    <Tooltip title={collapsed ? "Expand events panel" : "Collapse events panel"} placement="right">
      <IconButton
        size="small"
  ...
  return (
    <Box
      sx={{
        ...
        transition: `width ${PANEL_MS}ms ${PANEL_EASE}`,
        '@media (prefers-reduced-motion: reduce)': { transition: 'none' },
      }}
      data-testid={collapsed ? 'left-sidebar-collapsed' : 'left-sidebar'}
    >
      {/* Outside the clip, so the tab keeps hanging over the edge. */}
      {toggleTab}

      {/* Clip: the contents below keep their natural width and get revealed
          or covered as the shell animates. */}
      <Box sx={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
        <Box sx={{
          width, height: '100%', display: 'flex', flexDirection: 'column',
          alignItems: collapsed ? 'center' : 'stretch', py: collapsed ? 1 : 0,
        }}>
        {collapsed ? (
          <>
